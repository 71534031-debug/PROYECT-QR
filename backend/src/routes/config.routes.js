...
function createConfigRouter() {
  ...
  router.put('/', requireRoles('ADMIN'), async (req, res) => {
    ...
    if (existing.length > 0) {
      await pool.query(
        `UPDATE configuracion_institucional SET nombre_institucion = ?, logo_url = ?, firma_url = ?, cargo_autoridad = ?, nombre_autoridad = ?, nombre_app = ?, email_contacto = ?, telefono_contacto = ?, direccion = ? WHERE id = 1`,
      ...
      await pool.query(
        `INSERT INTO configuracion_institucional (id, nombre_institucion, logo_url, firma_url, cargo_autoridad, nombre_autoridad, nombre_app, email_contacto, telefono_contacto, direccion) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [nombre_institucion, logoUrl, firmaUrl, cargo_autoridad, nombre_autoridad, nombre_app || null, email_contacto || null, telefono_contacto || null, direccion || null]
      );
    }
    await pool.query(
