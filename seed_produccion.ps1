...
$actividades = @(
  @{nombre="Curso de Gestion de Proyectos TI"; tipo="Curso"; descripcion="Metodologias agiles"; fecha_inicio="2026-03-01"; fecha_fin="2026-03-30"; responsable="Decano Nacional"}
  ...
  @{nombre="Conferencia de Actualizacion 2026"; tipo="Conferencia"; descripcion="Nuevas tecnologias"; fecha_inicio="2026-05-15"; fecha_fin="2026-05-15"; responsable="Comite Organizador"}
  @{nombre="Seminario de Seguridad Estructural"; tipo="Seminario"; descripcion="Normas de seguridad"; fecha_inicio="2026-06-20"; fecha_fin="2026-06-22"; responsable="Jefe de Capitulo"}
  ...
  @{nombre="Capacitacion en Etica Profesional"; tipo="Curso"; descripcion="Capacitacion obligatoria"; fecha_inicio="2026-02-01"; fecha_fin="2026-02-28"; responsable="Comite de Etica"}
  @{nombre="Conferencia Actualizacion 2026-II"; tipo="Conferencia"; descripcion="Tendencias tecnologicas"; fecha_inicio="2026-09-10"; fecha_fin="2026-09-10"; responsable="Comite Organizador"}
)
$actIds = @()
foreach ($a in $actividades) {
  Start-Sleep -Milliseconds 500
  $r = Invoke-RestMethod -Uri "$BASE/actividades" -Method Post -Headers $headers -ContentType "application/json" -Body ($a | ConvertTo-Json) -TimeoutSec $TIMEOUT
  $actIds += $r.data.id
  Write-Host "  OK $($a.nombre) -> ID $($r.data.id)" -ForegroundColor Green
}
Write-Host "Total: $($actIds.Count) actividades" -ForegroundColor Yellow
...
$participantes = @(
  ...
  @{ai=2;n="Ana Sofia";a="Martinez Rios";td="DNI";nd="11223344";em="ana.martinez@email.com"}
...
foreach ($p in $participantes) {
  ...
  $aid = $actIds[$p.ai]
  $body = @{nombres=$p.n;apellidos=$p.a;tipo_documento=$p.td;numero_documento=$p.nd;email=$p.em;actividad_id=$aid} | ConvertTo-Json
  try {
    $r = Invoke-RestMethod -Uri "$BASE/participantes" -Method Post -Headers $headers -ContentType "application/json" -Body $body -TimeoutSec $TIMEOUT
    $partData += @{id=$r.data.id;actividad_id=$aid}
    Write-Host "  OK $($p.n) $($p.a)" -ForegroundColor Green
  } catch { Write-Host "  FAIL $($p.n) $($p.a): error" -ForegroundColor Red }
}
Write-Host "Total: $($partData.Count) participantes" -ForegroundColor Yellow
...
foreach ($pd in $partData) {
  ...
  try {
    $r = Invoke-RestMethod -Uri "$BASE/participantes/$($pd.id)/validar-apto" -Method Post -Headers $headers -ContentType "application/json" -Body (@{actividad_id=$pd.actividad_id} | ConvertTo-Json) -TimeoutSec $TIMEOUT
...
$plt = Invoke-RestMethod -Uri "$BASE/plantillas" -Method Post -Headers $headers -ContentType "application/json" -Body '{"nombre":"Certificado Oficial CIP"}' -TimeoutSec $TIMEOUT
$pltId = $plt.data.id
...
$cr = Invoke-RestMethod -Uri "$BASE/plantillas/$pltId/campos" -Method Put -Headers $headers -ContentType "application/json" -Body $camposJson -TimeoutSec $TIMEOUT
Write-Host "OK $($cr.data.campos.Count) campos configurados" -ForegroundColor Green
...
for ($i = 0; $i -lt $actIds.Count; $i++) {
  ...
  try {
    $r = Invoke-RestMethod -Uri "$BASE/certificados/generar" -Method Post -Headers $headers -ContentType "application/json" -Body (@{actividad_id=$aid;plantilla_id=$pltId} | ConvertTo-Json) -TimeoutSec $TIMEOUT
    if ($r.success) { $totalCert += $r.generados; Write-Host "  OK Act $($i+1) (ID $aid): $($r.generados)" -ForegroundColor Green }
    else { Write-Host "  FAIL Act $($i+1) (ID $aid): $($r.message)" -ForegroundColor Red }
  } catch { Write-Host "  FAIL Act $($i+1) (ID $aid): timeout/error" -ForegroundColor Red }
}
...
Write-Host "========================================"
Write-Host "  LOGIN: https://proyect-qr-backend.onrender.com"
Write-Host "  Credenciales: admin@cip.local / Password123"
Write-Host "========================================"